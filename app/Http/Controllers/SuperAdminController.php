...
class SuperAdminController extends Controller
{
    ...
    protected function SuperAdminLogin(Request $request)
    {
        ...
        if (auth('superadmin')->attempt($credentials)) {
            ...
            return response()->json(
                [
                    'status' => 'success',
                    'user' => ['user' => $user->name, 'role' => 'superadmin', 'email' => $user->email],
                    'accessToken' => $token,
                ],
                200,
            );
    ...
    protected function SuperAdminForgetPassword(Request $request)
    {
        // return $request->all();
        ...
        if ($user == null) {
            ...
            Mail::raw('Hey SuperAdmin : Your Code For Password Resseting is :' . $token, function ($message) use ($request) {
                $message->from(env('MAIL_USERNAME'), env('MAIL_FROM_NAME'));
    ...
    protected function SuperAdminResetPassword(Request $request)
    {
        $validator = Validator::make(request()->all(), [
            'email' => 'required|email',
            'token' => 'required',
        ...
        $user = SuperAdmin::where(['reset_token' => $request->token])->first();
        if (!$user) {
